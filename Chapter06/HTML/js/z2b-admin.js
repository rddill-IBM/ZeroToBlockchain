...
function listAssets()
{
  ...
  { console.log(_results);
    ...
    if (_results.result === 'success')
      {
        _str += '<table width="100%"><tr><th>Order Number</th><th>Created</th><th>Status</th><th>Buyer</th><th>Amount</th></tr>';
        for (each in _results.orders)
          {(function(_idx, _arr){
            _str += '<tr><td align="center">'+_arr[_idx].id+'</td><td>'+_arr[_idx].created+'</td><td>'+JSON.parse(_arr[_idx].status).text+'</td><td>'+_arr[_idx].buyer+'</td><td align="right">$'+_arr[_idx].amount+'.00</td></tr>';
